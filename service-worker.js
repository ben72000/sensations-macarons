// [v1382] CARNET DES TRAJETS — distance et temps de livraison repris automatiquement de TES livraisons. Demande de Ben : configurer l'adresse du client par rapport à son lieu de départ (labo) pour obtenir distance et temps de route. Choix de Ben parmi 3 options : « mémoire par adresse — 1re fois à la main, ensuite auto » — le seul qui ne FABRIQUE aucun chiffre (ces nombres alimentent computeDeliveryCost → rentabilité et marges ; une distance estimée y serait un troisième chiffre, v1339). État trouvé : un carnet d'adresses existait (settings.addressBook) et son pré-remplissage marchait, mais c'était une ÎLE — un seul écrivain (le formulaire manuel), les dizaines de livraisons déjà chiffrées dans les commandes ne l'alimentaient jamais ; choisir un client ne proposait rien ; une adresse tapée (vs cliquée dans la liste) ne déclenchait rien ; et le carnet ne disait jamais DEPUIS OÙ les distances étaient mesurées. Ajouté : (1) le POINT DE DÉPART est un réglage horodaté — les trajets mesurés avant un changement d'adresse sont SIGNALÉS, jamais purgés ; (2) le carnet APPREND des commandes réelles (_carnetTrajets, médiane par adresse — robuste au jour de bouchons, là où une moyenne se laisse tordre) ; (3) proposition aussi par CLIENT et sur adresse tapée ; (4) rien n'est jamais écrasé : champs vides → pré-remplissage annoncé, champs remplis → bouton Appliquer, et l'origine du chiffre est toujours dite ; (5) divergence carnet ↔ réalité remontée au lieu d'être arbitrée en silence ; (6) l'ancien écran devient un alias (deux écrans pour la même chose = deux vérités). Appariement d'adresses volontairement conservateur : rater une correspondance coûte une saisie, en inventer une coûte un chiffre faux. Suite v1382 : 38 assertions, dont la garde G1 qui interdit tout calcul géographique ou appel de routage.
